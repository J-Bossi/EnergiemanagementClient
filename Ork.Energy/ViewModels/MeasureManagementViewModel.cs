...
namespace Ork.Energy.ViewModels
{
    ...
    public class MeasureManagementViewModel : DocumentBase, IWorkspace
    {
        ...
        public IEnumerable<CatalogViewModel> FilteredCatalogs
        {
            get
            {
                CatalogViewModel[] filteredCatalogs = SearchInCatalogList().OrderBy(cat => cat.Name)
                    .ToArray();
        ...
        public void RemoveMeasure()
        {
            ...
            List<SubMeasure> allSubMeasures =
                m_Repository.SubMeasures.Where(sm => sm.ReleatedMeasure == measureViewModel.Model).ToList();
                ...
                .Contains(searchText)) ||
                                           c.Measures.Any(m =>
                                           {
                                               ...
                                               return employee != null &&
                                                      (employee.LastName + " " + employee.FirstName).ToLower()
                                                          .Contains(searchText);
                                           }))));
        }
        ...
        private void OpenCatalogEditDialog(object dataContext)
        {
            ...
            OpenEditor(m_MeasureViewModelFactory.CreateCatalogEditViewModel((CatalogViewModel) dataContext,
                RemoveCatalog));
        }
        ...
        private void InitializePlot() // Organisatorische Grafik
        {
            ...
            {
                LegendTitle =
                    TranslationProvider.Translate(Assembly.GetExecutingAssembly(), "TitleMeasureManagementViewModel"),
            };
            ...
            // Erzeugt Daten für organisatorische Grafik
        {
            ...
            IOrderedEnumerable<MeasureViewModel> sortedList = measures.Where(
                m =>
                    (m.DueDate <= (lastDayofMonth)) &&
                    (m.DueDate >= (lastDayofMonth.Subtract(new TimeSpan(200, 0, 0, 0)))))
                .OrderBy(m => m.DueDate);
            ...
            foreach (int month in monthArray)
            {
                int completed = measures.Count(m => m.Status == 2 && m.DueDate.Month == m.EntryDate.GetValueOrDefault()
                    .Month && m.EntryDate <= m.DueDate && m.EntryDate.GetValueOrDefault()
                        .Month == month);
                int delayed =
                    measures.Count(m => m.Status == 2 && m.EntryDate > m.DueDate && m.EntryDate.GetValueOrDefault()
                        .Month == month);
                int completedPrior =
                    measures.Count(m => m.Status == 2 && m.DueDate.Month > m.EntryDate.GetValueOrDefault()
                        .Month && m.EntryDate <= m.DueDate && m.EntryDate.GetValueOrDefault()
                            .Month == month);
